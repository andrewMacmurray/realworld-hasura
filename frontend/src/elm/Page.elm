...
view user model =
    case model of
        ...
        NewPost model_ ->
            Element.map NewPostMsg (authenticated NewPost.view model_ user)

        NotFound ->
            NotFound.view


authenticated : (User.Profile -> subModel -> Element msg) -> subModel -> User -> Element msg
authenticated view__ subModel user =
    case user of
        User.Guest ->
            NotFound.view
